...
RUN apt-get update && apt-get install -y gnupg \
    ...
    && echo "deb  http://dl.softplc.com/gatecraft-repos buster embedded" > /etc/apt/sources.list.d/gatecraft.list \
...
#CMD [ "./softplc", "-daemon", "&&", "/usr/sbin/sshd", "-D"]

# CMD [ "/SoftPLC/run/runsplc"]
# CMD [ "/usr/sbin/sshd", "-D" ]
#   && echo "export VISIBLE=now" >> /etc/profile
#   && echo /SoftPLC/run > /etc/ld.so.conf.d/softplc.conf
